%%
outpath = 'results';
rawpaths = dir('..\20260213 上海 纪小帅② 9张荧光5标扫描（9-9-9）黄中庆 #SWR03106636\*.svs');
%%
chunk_a = 1024;
vx_log2 = (0:0.05:8)';
vx = 2 .^ vx_log2 - 1;
...
for nn = 1:ns
    ...
    parfor (tt = 1:nt, 16)
        ...
        tmp = m(:, :, 2);
        cdist(tt).gcsh = histcounts(tmp, [vx; 256]);
        cdist(tt).gcsh_filt = histcounts(imgaussfilt(tmp, 1), [vx; 256]);

        tmp = m(:, :, 4);
        cdist(tt).rtn4 = histcounts(tmp, [vx; 256]);
        cdist(tt).rtn4_filt = histcounts(imgaussfilt(tmp, 5), [vx; 256]);

        tmp = m(:, :, 6);
        cdist(tt).bmper = histcounts(tmp, [vx; 256]);
        cdist(tt).bmper_filt = histcounts(imgaussfilt(tmp, 5), [vx; 256]);
...
for nn = 1:ns
    ...
    plot(vx_log2, mean(ds(nn).chan_dist.gcsh(isin, :)));
    plot(vx_log2, mean(ds(nn).chan_dist.gcsh_filt(isin, :)));
    set(gca, 'XLim', [0, 8], 'YScale', 'log', 'YLim', [1e-3, 1e6]); grid on;
    title('GCSH');

    subplot(2, 4, 4); hold on;
    plot(vx_log2, mean(ds(nn).chan_dist.rtn4(isin, :)));
    plot(vx_log2, mean(ds(nn).chan_dist.rtn4_filt(isin, :)));
    set(gca, 'XLim', [0, 8], 'YScale', 'log', 'YLim', [1e-3, 1e6]); grid on;
    title('RTN4');

    subplot(2, 4, 5); hold on;
    plot(vx_log2, mean(ds(nn).chan_dist.bmper(isin, :)));
    plot(vx_log2, mean(ds(nn).chan_dist.bmper_filt(isin, :)));
    set(gca, 'XLim', [0, 8], 'YScale', 'log', 'YLim', [1e-3, 1e6]); grid on;
    title('BMPER');
